...
std::string HttpRequest::process_request(std::string message) {
	if (message.substr(0, message.find("/") - 1) != type ) {return "400 Bad Request";}
	file_path += message.substr(message.find("/"), message.find("H") - 1);
	if (message.substr(message.find("HTTP"), message.find("\r") - 1) != http_ver) {return "505 HTTP version not supported";}
